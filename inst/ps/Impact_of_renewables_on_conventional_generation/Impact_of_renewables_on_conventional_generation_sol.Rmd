...
setwd("D:/Benutzer/Dokumente/Dokumente/Studium/03_Semester/Masterarbeit/0_Mein_Problemset/0_Problemset/2_Versuch")
ps.name = "Impact_of_renewables_on_conventional_generation"; sol.file = paste0(ps.name,"_sol.Rmd")
libs = c("ggplot2", "haven", "dplyr", "sandwich", "AER", "stats","stargazer","car", "patchwork") # character vector of all packages you load in the problem set
...
#knitr::opts_chunk$set(warning = FALSE, message = FALSE, error = TRUE) beim knitten
```
#>
...
To read `.dta` files with *R* we can use the `read_dta()` function of the package `haven`. To do so we need to **load the required `haven` package**. This is done using the function `library()`. The name of the package has to be passed to the function as a parameter.

```{r "1.1.1",eval=FALSE}
library(haven)
```

After the package is loaded, we can use all functions included in this package. 

To load the data, we need to **call the function `read_dta()`** and pass the name and the path of the file to the command. If the file is located in the current working directory (as we assume here) we only have to pass the file name *file_name.dta*. Do not forget to use parentheses for the name.

```{r "1.1.2",eval=FALSE}
read_dta("file_name.dta")
```

To access the loaded data, we need to store it into a variable in the workspace.  Therefore, we have to **assign a variable** to the `read_dta()` command. For example, if we want to name the variable `data` the corresponding code has to look like this:

```{r "1.1.3",eval=FALSE}
data = read_dta("file_name.dta")
```

By executing this line of code, the dataset is loaded and stored inside the variable `data`. 

If you have troubles or you are just curious about some commands, it is always helpful to read the corresponding documentation. For example, the [read_dta() documentation](https://haven.tidyverse.org/reference/read_dta.html).
...
```
#< award "Beginner"
Congratulations, you successfully loaded your first dataset!
#>
...
#< quiz "origin date of stata"
question: What is the right origin date used by *stata's* Julian date?
sc:
- 01.01.1960*
- 01.01.2000
- 31.12.1899
...
Earlier, we showed how to access certain columns using the `$` syntax. Although this works universally with base R, it can become confusing as more functions and commands are added together. Furthermore, we can only select one column at a time. 

Fortunately, the command `select()` offers a solution by allowing us to choose a portion of the data frame columns. To do so the command needs the name of the data frame and the names of the wanted columns as an argument. For example, to select the `day` and `hour` columns from the `dat` dataset, we can write:
```{r "1.1.12",eval=FALSE}
# The first argument has to be the data frame and the other arguments are the names of the columns to select.
select(dat, day, hour)
```

The data frame we get back from this command is equal to the data frame we get if we write:
```{r "1.1.13",eval=FALSE}
# Here the bind_cols() command binds the two separate columns together and names the columns as given. 
bind_cols(day = dat$day, hour = dat$hour)
```

You see that the first method looks much cleaner and is easier to read. If you want to learn more about the function `select()`, please read [here](https://dplyr.tidyverse.org/reference/select.html).  

Similarly, if we need to choose specific rows (observations), the `filter()` function is the easiest approach. This function requires the data frame name as the first argument and a logical condition as the second. The function creates a new subset with all the observations that meet the requirement condition.

For instance, to retain all rows with an `hour` of either 12 or 24 (logical condition: `hour == 12 | hour == 24`), we can use the following code:
```{r "1.1.14",eval=FALSE}
filter(dat, hour == 12 | hour == 24)
```

If you want to learn more about the function `filter()` please read [here](https://dplyr.tidyverse.org/reference/filter.html).
#>


#< info "How to write code using pipes (%>%)."

We will now take a look on how to use the `%>%` operator. This will showcase the potency of the `dplyr` commands.

Pipes `%>%` are a very helpful way to write more complex code in a way that is easy to read and understand. Normally, when writing code chunks including many different functions, one has two options. First, it is possible to write the code in different lines with intermediate results or second, one has to combine all functions in one command which is performed from the inside to the outside. Both ways have different disadvantages. By using the first method more useless variables are stored in the working environment. By using the second method it is difficult to understand the code easily.

Using pipes offers a third method that combines the best of both strategies to eliminate the disadvantages of each individual method.

You now see three different examples of code that generate the same output:

The first example follows the approach of writing out every line of code separately and storing the intermediate result.
```{r "1.1.15",eval=FALSE}
...
As you see the code is in general readable, but it is not very efficient to write code like that.
...
The command is now much smaller and easier to execute but commands like this can become very confusing.

Normally you would neither write code in the first nor the second form but use pipes instead.
```{r "1.1.17",eval=FALSE}
...
Here you can read the function of the code from top to bottom. Especially when you have complex code, this is the method of choice. Note that pipes directly pass intermediate results to the next command. Functions that usually need data as their first argument use these passed results and thus require no additional data input.

If you want to know more about pipe operators `%>%` please read [here](https://style.tidyverse.org/pipes.html).

**Pro tip**: You can easily write the pipe operator `%>%` with the shortcut `Ctrl + Shift + m`.
#>
...
Nice! We now know which observations are missing. Indiana Jones would be proud.
#>
...
(Based on Bushnell and Novan, 2021c, README.docx)

In this info box I will give you a brief description of the variables (except the already explained time variables) you see in the summary above. A detailed explanation for the more complex variables will follow when they appear in our analysis. 
...
CAISO reports four prices for various price zones called "default load aggregation points" (DLAPs). Each DLAP price includes three components: the energy price (99% of the total price and the same across all DLAPs), the congestion component and the loss component (both different for each DLAP). Bushnell and Novan calculate the average of the various DLAPs by assigning different weights to obtain the total prices for the different components (2021a, p. 267).  
...
The function `mutate()` from the package `dplyr` allows to generate, change and remove columns in a dataset. Simply give the function the data frame as the initial argument, then supply the names of the old (to modify) or the new columns (to add), and their new values. These values may involve mathematical expressions, alongside other variables or functions. Remember to save the command output in a new dataset or overwrite the old one. 
```{r "1.2.2",eval=FALSE}
# The first argument has to be the data frame and the other arguments are the new or the modified columns.
dat_new = mutate(dat, solar_wind = solar + wind) #creates a new column 'solar_wind' which contains the sum of the column 'solar' and the column 'wind'
```
...
dat_new$X = dat_new$renewables - dat_new$solar - dat_new$wind
```

Note, that using `mutate()` instead of the `$` syntax allows the creation of pipe chains.
If you want to learn more about the mutate command, please read [here](https://dplyr.tidyverse.org/reference/mutate.html).
#>
...
The whole `ggplot2` package works according to multi-layer principle. The basis of every plot created with `ggplot2` is the ggplot object which is created with the command `ggplot()`. As an argument, you can already give this function the data and the visual properties of objects in the plot (called aesthetics `aes()`). Next, `geoms` can be added as layers to the ggplot object to plot the data. These `geoms` can also have arguments for data or aesthetics. If they do, they replace the arguments in ggplot(). Lastly, you may add additional layers of properties or style elements.

```{r "1.2.7",eval=FALSE}
ggplot(dat, aes(x = date, y = solar_cap)) +   #creates a ggplot object based on the dataset 'dat' and defines the aesthetics
  geom_line()+                                #adds a layer containing a line plot
  geom_point(color = "red")                   #adds a layer containing a point plot and sets the color of the points
  xlab("Date")+                               #labels the x-axis
  ylab("Solar capacity")                      #labels the y-axis
```

This is just a very basic explanation and example of the entire range of possibilities offered by `ggplot2`, but it is enough for our usage. If you want to learn more, please read [here](https://ggplot2.tidyverse.org/).
...
ggplot(dat, aes(x = date, y = daily_solar)) +
  geom_line()+
  xlab("Year")+
  ylab("Daily solar production in GWh")+
  labs(title = "Solar production over observation period")
...
As the title of the info box suggests, the `group_by()` function allows you to define grouping variables that group your data frame. On grouped data frames you can perform useful functions like `summarise()`. The command takes a data frame and grouping variables as arguments, as you can see in the example below.

```{r "1.2.10",eval=FALSE}
dat_grouped = group_by(dat, hour, year) #arguments are the data frame 'dat' and the grouping variables 'hour' and 'year' 
```

After using the `group_by()` function, the appearance of the data frame remains the same as the original. However, it is now grouped.
If you want to learn more about `group_by()`, please read [here](https://dplyr.tidyverse.org/reference/group_by.html).

One of the most frequently used commands on grouped datasets is the `summarise()` function. This creates a new data frame that summarises the variables of each group in a way that is passed as a parameter to the function. 

```{r "1.2.11",eval=FALSE}
dat_new = summarise(dat_grouped, load_sum = sum(load), solar_wind = solar + wind) 
```

Perhaps you may notice a similarity to `mutate()`. A new data frame is created containing the grouping variables and this data frame is mutated separately for each group to summarise the data in it.

If you want to learn more about `summarise()`, please read [here](https://dplyr.tidyverse.org/reference/summarise.html).

As you might imagine, these two commands are an excellent case to use the pipe operator `%>%`.
```{r "1.2.12",eval=FALSE}
data_new = dat %>% 
  group_by(hour, year) %>% 
...
As the prices in the electricity market are, as in other markets, a consequence of supply and demand, it is possible to easily illustrate the corresponding price-building process.

The figure below shows on the x-axis the capacities of all available generators in our example market. The y-axis shows the variable energy-producing costs of these generators (since we assume the variable costs to be constant for each individual generator, they are equal to their marginal costs). As you can see the different generators on the x-axis are sorted by their variable costs starting from the lowest to the highest running costs. 


![](merit-order.png)

Since in this price-building process generators are assumed to produce electricity as long as the price they get is greater or equal to their variable costs, it is possible to draw the demand into the diagram. This then shows the most recently needed power generator in the market (the marginal generator). The costs of this generator determine the market clearance price for our example market. Consequently, this market clearance price is the overall market price for electricity at this demand. 

This whole price-building process together with the underlying assumptions is called **merit-order-principle**. As you can see, if for example the input capacities of renewable generators increase, less energy from high marginal cost generators is needed.  Therefore, if the new recently needed power generator has lower marginal costs, also the market price for the energy decreases. This effect is called **the merit-order effect of renewable generation** (Sensfuß et al., 2008).
#>
...
question: How do you think the average hourly real-time market price for electricity changed from 2013 to 2016?
sc:
- the price has fallen in all hours
- the price has remained roughly the same
- the price has risen in all hours
- the price has fallen in most hours but not as much in the morning and evening hours*
- the price has fallen in the daylight hours and risen in the night hours
...
```
#< award "Rolling Stone"
Bushnell and Novan start their analysis with this figure as a first initial evidence for increasing price effects. Therefore, it sets the ball rolling or as we say in german: "Den Stein ins Rollen bringen".
#>
...
**Task** Just `check` the chunk to generate the plot.
```{r "1.2.17",fig.width=10.5,fig.height=7}
#< task
ggplot() +
...
A helpful way of examining how a variable affects another is regression analysis (Wooldridge, 2013, pp. 22-117; Stock et al. 2019, pp. 143-246; Verbeek, 2004, pp. 7-15). If we assume that the effect, we are searching for is linear we can build a model like this:

$$
y_t = \beta_0 + \beta_1 x_{1t} + \varepsilon_t
$$

As you can imagine, this type of model is called the **"simple linear regression model"** (Stock et al. 2019, p. 145). $y_t$ is called the dependent or explained variable. Since we are working with data that changes over time, the index $t$ refers to the observation and can range from $t = 1,2,..., T$. $x_{1t}$ represent the independent or explanatory variable. $\beta_0$ and  $\beta_1$ are a set of fixed coefficients that determine an offset and how the explanatory variable affects the outcome. Lastly, there is a term called error or disturbance ($\varepsilon_t$) which accounts for random fluctuations that cannot be explained by the explanatory variables of the model.

We aim to determine the betas ($\beta_k$) to study the effect of the explanatory variable on the explained variable. Typically, one observes the dependent variable and its corresponding independent variables, but not the betas. Hence, a regression analysis is necessary. A regression aims to discover the corresponding betas for a given model and a set of dependent and independent variables. Several techniques are available to achieve this. The fundamental method we will use is the **ordinary least squares (OLS)** estimation.

Additionally, to the simple linear regression model, there is a more advanced model containing a wider set of explanatory variables ($x_{kt}$) and therefore also more betas ($\beta_k$):
$$
y_t = \beta_0 + \beta_1 x_{1t} + \beta_2 x_{2t} + ...+ \beta_k x_{kt} + \varepsilon_t
$$
This regression model is called **"multiple linear regression model"** (Stock et al. 2019, p. 219).
This type of regression model can also be written using matrices (Verbeek, 2004, p. 12):

$$
y = X \beta + \varepsilon
$$
The expression contains a vector, denoted by $y = (y_1,...y_T)'$, which includes all the dependent variables. The vector of the true coefficients is represented by $\beta = (\beta_0,...,\beta_k)'$, and the disturbances are held in the vector $\varepsilon = (\varepsilon_1,...\varepsilon_T)'$. The matrix X containing the independent variables looks like this:
...
As **estimated** values are denoted with hats ($\hat{y}$, $\hat{\beta}$, $\hat{\varepsilon}$), the solution we are interested in, which we get from the OLS regression is $\hat{\beta}$. Generally, we calculate $\hat{y}$ as $X \hat{\beta} + \hat{\varepsilon}$ and $\hat{\varepsilon}$ is defined as $\hat{\varepsilon} = y-\hat{y}$. These $\hat{\varepsilon}$ are known as the **residuals** or estimated values of the disturbance. We can see from the definition of residuals that $\hat{y}$ is close to the real $y$ if $\hat{\varepsilon}$ is small. That also means, that $\hat{\beta}$ is also close to the true $\beta$ when $\hat{\varepsilon}$ is small. This is the main principle of OLS estimation. By reducing the sum of squared residuals, we aim to obtain the best estimates for $\beta$ (Wooldridge, 2013, p. 73). 

$$
\hat{\beta} = \arg \min \sum_{t=1}^{T}{\hat{\varepsilon}_t^2}
$$
The solution of this minimization problem appears like (Verbeek, 2004, p. 13; Kennedy, 2008, p. 48):

$$
\hat{\beta} = (X'X)^{-1}X'y
$$
Thankfully, usually one does not need to calculate the OLS estimators using this formula. Instead, depending on the particular issue, one can utilize a variety of *R* commands.

The OLS regression (multiple linear regression solved with OLS estimation) makes some assumptions about the regression model and the underlying data when estimating $\hat{\beta}$. Not all of these assumptions are necessary, but some are essential to achieve a meaningful unbiased solution.  For now, I will not delve into these assumptions in more detail, but some will resurface and be explained in greater depth later on. If you are keen to learn more about these assumptions, please read [here](https://www.albert.io/blog/key-assumptions-of-ols-econometrics-review/) or read some of the cited textbooks.
...
The function `lm()` of the package `stats` allows to perform OLS regressions. As a parameter, this function needs a regression model and a dataset containing the observations which include the dependent and the independent variables.

```{r "2.1.1",eval=FALSE}
reg = lm(y ~ x1 + x2, data = data_set)  #the model can be directly passed to the function

reg_model = formula(y ~ x1 + x2)        #or the model is defined before as a formula
reg = lm(reg_model, data = data_set)    #and than passed as a variable
```

As you can see, the use of `lm()` is straightforward. Notice that if you want to add more variables just combine them with `+`. Also, you do not have to add the factors $\beta_k$ nor the error term to the model. 

If you want to learn more about `lm()`, please read [here](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/lm).
#>
...
reg_short = lm(rtm_total ~ solar + wind, data = dat_h12)
```
#< award "Regression greenhorn"
Well done, this was the first of many regressions you will perform in this problem set.
#>
...
#< quiz "reg_short results"
question: What effects of the solar and wind production would you expect on the real-time market price at 12 pm? 
sc:
- solar has no effect, wind has a negative effect
- solar has no effect, wind has a positive effect
- solar has a negative effect, wind has no effect
- solar has a positive effect, wind has no effect
- both have no effect
...
Considering a linear regression model $y = \beta_0 + \beta_1 x_{1} + \beta_2 x_{2} + ...+ \beta_k x_{k} + \varepsilon$.

A **t-test** for regression coefficients is a sort of [hypothesis test](https://en.wikipedia.org/wiki/Statistical_hypothesis_testing) (Verbeek, 2004, pp. 23-25; Stock et al., 2019, pp. 247-253) where 

H0: $\beta_k = 0$ and H1: $\beta_k \neq 0$.

The corresponding test statistic for the t-test looks like

$$
t_k = \frac{\hat\beta_k}{\hat{sd}(\hat\beta_k)}
$$
and calculates the associated **t-value $t_k$**. The t-value therefore indicates how many standard deviations the coefficient is away from zero. While $\hat\beta_k$ is the estimator of $\beta_k$, $\hat{sd}(\hat\beta_k)$ is the standard deviation of the estimator. Like the two variables in the fraction also the t-value itself is a random variable based on $\varepsilon$ and explanatory variables.

Since we know the distribution of the test statistic when H0 and all assumptions of the linear regression model are true, we can calculate the probability of finding the realized or more extreme test statistic when H0 is assumed. This probability is called the **p-value** (Stock et al., 2019, p. 249).

If the p-value of a t-test statistic is below a certain threshold $\alpha$, the null hypothesis can be rejected at the $\alpha$ significance level. Common thresholds are 0.1%, 1% and 5%. In *R* these p-values are marked with three stars, two stars and one star to make it easier to see the significance. A significant effect is usually said to exist when the H0 hypothesis can be rejected by at least 5%.


While the statistical measure presented above shows how significant the effect of each individual coefficient is, the now introduced **$\text{R}^2$ or R-squared value** gives a measure of the quality of the whole regression model (Stock et al., 2019, p. 223). This measure is also called the **coefficient of determination** and has several definitions depending on the application. In our case, it describes how much of the total variation in the dependent variable $y$ can be explained by our model and by variations in the explanatory variables $X$. The most general definition is (Wooldridge, 2013, p. 202):

$$
R^2 = 1 - \frac{SSR}{SST} = 1- \frac{\sum_{t=1}^T{\hat \varepsilon^2_t}}{\sum_{t=1}^T{(y_t-\overline y)^2}}.
$$


In this context, **SSR** is the sum of squares of the residuals or the residual sum of squares and **SST** is the total sum of squares. $R^2$ can typically range from 0 to 1, where 0 means that the model cannot explain any variation in $y$ and 1 means that all variation can be explained. Because OLS minimizes the sum of squares (SSR), it automatically maximizes $R^2$.

A modified version of the $R^2$-value is the **adjusted** $R^2$-value (Stock et al., 2019, p. 223). This one adjusts for the number of predictors in the regression model. As more and more explainable variables are added to a model, $R^2$ will automatically increase (by chance). This is a temptation to add many variables, which can lead to over-fitting problems. Therefore, the adjusted $R^2$ corrects for this random increase, helping to prevent over-fitting. The adjusted $R^2$ can be interpreted as a less biased estimator of the population $R^2$.
#>
...
The exogeneity assumption is one of the main assumptions of OLS regression and assumes that the error term ($\varepsilon_t$) has no correlation with all explanatory variables ($X$) and that its average value is zero. This is typically expressed as (Stock et al., 2019, p. 213):  
$$
\text{E}[\varepsilon|X]=0
$$

The more important factor here is that the expected value of $\varepsilon$ is independent of $X$, not the zero on the right. If this condition is satisfied, we call the error term **exogenous**. When it is not, we call it **endogenous** (Wooldridge, 2013, p. 87). 

This assumption of exogeneity is crucial for the OLS estimator to be unbiased and therefore one has to think about possible endogeneity problems in the regression model and how to deal with them.
#>
...
#< quiz "bias of confounder "

question: Based on the relationship chart, which direction of the bias associated with the coefficient of solar do you expect in the short regression? 
sc:
- I expect the biased coefficient to be biased towards zero (underestimating the effect).
- I expect the biased coefficient to be biased away from zero  (overestimating the effect).*
...
An important distinction that often leads to confusion or misinterpretation is the difference between [correlation and causality](https://www.abs.gov.au/statistics/understanding-statistics/statistical-terms-and-concepts/correlation-and-causation). These are two crucial concepts in statistics and research. 

Correlation refers to a statistical connection between two variables, without implying that one causes the other. If two variables are correlated, it indicates that when one variable changes, the other typically changes in a particular way. For example, as sales of ice cream increase, more people go swimming. Nevertheless, this does not imply that purchasing ice cream causes people to go swimming (or the other way around).

Causality is the relationship between variables, where changes in one variable cause changes in the other. For instance, when the temperature of water rises, it boils. This is a simple cause (temperature increase) and effect (water boiling) relationship.

It is important to understand that while we can use a correlation coefficient (-1 to 1) to measure correlation, understanding the underlying mechanism is key when establishing causality, and this often requires experimental data.

It is important to recall the well-known phrase, "Correlation does not imply causation," as a connection between two factors does not necessarily mean that one causes the other. More extensive investigation and proofs are often required to validate a causal relationship, as there could be a third influencing factor or the association may be a coincidence.

A website that illustrates the difference between the two concepts very well by presenting unexpected correlations where causality is clearly not given can be found [here](https://www.tylervigen.com/spurious-correlations).
...
beta_load * cov_solar_load / var_solar
```
...
#< quiz "multiple of 95 percent confidence interval"

question: Which multiple of the standard error is used to calculate the 95% confidence interval?
sc:
- 1.96*
- 2.33
- 0.99
...
___(reg_long)
#>
conf_upper = coef(reg_long) + 2*se
conf_upper

conf_lower = coef(reg_long) - 2*se
conf_lower

confint(reg_long)
```
#< award "Confident intervals"
Well done! As a reward you get a small AI generated poem about confident intervals:

In the realm of stats where numbers dance, confidence intervals give truth a chance.
A range so bold, a guess refined, in its bounds, true mean we find.

With margins set from low to high, we capture where the truths may lie.
Not absolute, but rather a guide, where estimates and accuracy collide.

A 95% confident embrace, holds the population’s true face.
Not just a single point so bare, but an interval showing care.

So here’s to the range that gently reveals, the heart of data, the facts it conceals.
In confidence intervals, our trust we instill, for they offer insight, not just a thrill.
#>
...
So far, we have seen one of the assumptions of OLS regression, the assumption of exogeneity. Now, let us take a look at another assumption called the **spherical errors assumption** (Kennedy, 2008, pp. 112-122). In mathematical terms, this assumption looks like:
$$
...
where $\sigma^2$ is the constant variance of the error terms and $I$ is the identity matrix.

This assumption basically is a combination of two smaller assumptions.

First there is the **Homoscedasticity assumption** (Wooldridge, 2013, p. 51):
$$
Var(\varepsilon_t|X)=\sigma^2 \hspace{1cm}\forall t = 1,2,\dots,T
$$
This means that all error terms $\varepsilon_t$ are assumed to have the same constant variance $\sigma^2$. This property is called homoscedasticity. The opposite would be called **heteroscedasticity** (Verbeek, 2004, pp. 82-83).

The second assumption included in the spherical errors assumption is the **non-autocorrelation assumption** (Verbeek, 2004, pp. 97-98; Wooldridge, 2013, p. 353):
$$
Cov(\varepsilon_t,\varepsilon_s|X)=0 \hspace{1cm}\forall t \neq s
$$
Hence, the observed errors should not be correlated with each other.

Unlike the exogeneity assumption, a violation of the spherical errors assumption does not lead to incorrect estimates of the coefficients, but of the associated standard errors (Kennedy, 2008, p. 113). Consequently, if the assumption is incorrect, the errors must be corrected.
...
nw = NeweyWest(reg_long,lag = 7, prewhite = FALSE)
nw_error = sqrt(diag(nw))

stargazer(reg_long, reg_long, type = "text", se = list(NULL, nw_error), column.labels = c("Normal SE", "HAC SE"), digits = 5)

```
#< award "HAC robust errors"
Superb, you have now finished the chapter about standard errors, confident intervals and error adjustment. That means we are well prepared for the following analysis.
#>
...
Source: (Bushnell & Novan, 2021b, p. 9.) 
...
#< quiz "supply-driven price trend"
question: Do you think nuclear power plants are assumed to drive a supply-driven price trend?
sc:
- Yes
- Yes, but we already control for this trend
- I do not know
- No*
...
#< quiz "neglecting concern"

question: Why do you think may Bushnell and Novan propose to neglect this first concern?
sc:
- because we already control for drivers of long-run changes in RTM prices
- because growth in renewable capacity was not mainly influenced by market forces
- both*
- It is a trick question! It cannot be neglected
...
Energy producers or grid operators may sometimes intentionally reduce renewable energy output, even if there is a higher production possible. This restriction is called **curtailment**. There are several reasons for this action.

One possible reason for the curtailment of energy output from renewable sources could be to maintain the grid stability when supply exceeds demand. 2020 in the CAISO grid around 5% of the utility-scale solar production was curtailed (EIA, 2021). In spring, curtailment is usually higher due to increased solar energy production. In this season, temperatures are not as cold, resulting in less heating demand. At the same time, temperatures are not high enough to require air conditioning.

You can read more about the energy curtailment in the CAISO market [here](https://www.eia.gov/todayinenergy/detail.php?id=49276).

A second, less frequent reason for renewable energy curtailment is to keep the market prices for electrical energy high. In some hours it probably makes sense for a producer to curtail easy to regulate and low-cost energy sources in order to  provide a higher energy price for power plants with higher costs but less flexibility, for example coal power plants.
#>
...
In *R* a `for()` loop is a function that performs the same set of commands multiple times. The structure of a `for()` loop looks like this:
...
Commands enclosed in curly braces `{}` are repeated, with the `for()` argument defining the number of repetitions. The loop requires a `vector`, which can be a sequence of numbers or a vector containing items. The loop executes the commands in the block for each entry in the vector. The current entry can be used as a variable with a name defined in `variable`.

For example, if one wants to add all numbers from 1 to 100 the corresponding loop looks like this:

```{r "3.2.3",eval=FALSE}
gauss = 0

for (variable in 1:100) {
  gauss = gauss + variable
}
```

As you might know, the result can be easily determined with the Gaussian formula $\sum_{k=1}^n k = \frac{n(n+1)}{2}=\frac{100 \cdot 101}{2} = 5050$.
#>
...
With `ifelse()` it is possible to return different outputs `output_yes` or `output_no` depending on a logical input `condition`.

```{r "3.2.4",eval=FALSE}
ifelse(condition, output_yes, output_no)
```

For example, the following code tests if a variable `x` is greater than `4`. If this is the case a variable `y` is set to `1` and otherwise to `0`. To demonstrate that it is also possible to return different data types, another variable `z` is defined that carries a string that verbalises the result of `y`.

```{r "3.2.5",eval=FALSE}
x = 5
y = ifelse(x > 4, 1, 0)
z = ifelse(y == 1, "x is greater than 4", "x is less or equal as 4")
...
With `paste()` you can add strings together to form a new string. As an argument you just insert the different strings and specify with `sep = "..."` how they should be separated.  
```{r "3.2.6",eval=FALSE}
str1 = "Hello"
str2 = "World"
string_new = paste(str1, str2, sep = "_") #separates the two strings with "_"

#string_new contains a string "Hello_World"
```

It is also possible to concatenate vector entries after converting them to characters.
There the argument `collapse = "..."` is used to separate the results.

```{r "3.2.7",eval=FALSE}
paste(m, 1:5, sep = "", collapse = "+")

#result would be "m1 + m2 + m3 + m4 + m5"
```
#>
...
#< quiz "wind and gas_price price effect"

question: Do you have a clue how the estimated price effect for these two variables will look like?
sc:

- Both variables will have a positive effect on RTM prices.
- Both variables will have a negative effect on RTM prices.
- Daily wind will have a negative effect on RTM prices and the gas spot market price a positive one. *
- Daily wind will have a positive effect on RTM prices and the gas spot market price a negative one.
- Daily wind will have a similar effect on RTM prices as daily solar and the gas spot market price a positive one.
- Daily wind will have a similar effect on RTM prices as daily solar and the gas spot market price a negative one.
...
The unit **MMBtu (million British thermal units)** is a unit used for heat energy. One British thermal unit is defined as the amount of heat required to raise the temperature of one pound of water by one degree Fahrenheit (EIA, 2023a). This definition is quite similar to the definition of a calorie but uses imperial units instead of metric ones.

There are several definitions for the exact value of a Btu, as mass and heat capacity depend on the initial temperature. The definition used by the U.S. Energy Information Administration (EIA) is at an initial temperature of 59°F (15°C) (EIA, 2023a). To convert from Btu to Joule, use the following formula:

$$
1 \text{Btu} \approx 1054.80 \text{J} \approx 0.293 \text{Wh}_{\text{termic}}
$$
#>
...
#beta solar = -0.40
effect_solar = -0.40 * (56.8 - 15.1)
effect_solar

#beta wind = -0.22
effect_wind = -0.22 * (37.9 - 34.6)
effect_wind

#beta gas = 4.23
effect_gas = 4.23 * (2.51 - 3.73)
effect_gas
...
```{r "3.3.4",fig.width=10.5,fig.height=7}
#< task
...
#< quiz "behavior of nuclear and other renewables"
question: Is the behavior of nuclear power and other renewables surprising?
sc:
- Yes, I think we expected other results. 
- No, I think that is the result we expected. *
...
```
#< award "Almost there"
Well done, this illustration will lead us to our first big result and thus to the end of this chapter.
#>
...
#< quiz "problematic energy sources"

question: Which energy sources might be a problem for our analysis?
sc:
- nuclear and the other renewables 
- large hydro and imports *
- thermal and imports
...
With the help of `case_when()` you can combine multiple `if_else()` statements in one elegant command. As an argument, the function needs some logical statements (cases) that lead to a `TRUE` or `FALSE` answer. With a tilde operator, a return value for each statement can be assigned. Every case is evaluated sequentially and the first match determines the output. If no case matches, the `.default`-value is used. 
...
x = round(runif(1, 0, 5))   #draw a random full number between 0 and 5
y = round(runif(1, 0, 5))   #draw a random full number between 0 and 5

case_when(
  x == y ~ "equal",
  x < y ~ "x smaller",
  (x > y) & ((x + y) %% 2 == 0) ~ "x greater and sum even",
  .default = "neither equal nor x smaller nor x greater and sum even"
)
```

If you want to learn more about the `case_when()` command, please read [here](https://dplyr.tidyverse.org/reference/case_when.html).
#>
...
```{r "4.6",fig.width=10.5, fig.height=7}
#< task
library(ggplot2)
library(patchwork)
...
```{r "4.7",fig.width=10.5, fig.height=7}
#< task
linesize = 1
color = c("blue","red","green")
...
#>
```
#< award "(E)Mission control"
The next big result is now within reach. Well done! If you need a short break, you can watch a [video of an astronaut in the ISS singing David Bowie's "Space Oditty"](https://www.youtube.com/watch?v=KaOC9danxNo). 
#>
...
#< quiz "question of capacity"
question: Is it easier/more sensible to focus on capacities greater or smaller than the actual ones or is there no clear answer?
sc:
- greater capacities than the observed ones
- smaller capacities than the observed ones*
- it does not matter
success: Great, your answer is correct!
failure: Try again.
#>
...
```
#< award "Formula king"
Well done! Here, a math joke as a reward: What do you call an empty parrot cage? A polygon (A Polly gone).
#>
...
A **tobit model** is a regression model, where the range of the dependent variable is censored (Greene, 2002, p. 764). This type of model makes sense if some constraints about the data are known. For example, when one wants to estimate the demand for tickets to a football event, then the natural upper boundary is the capacity of the stadium. Such constraints can appear at the upper and the lower boundary. Mathematically a Tobit model with a lower boundary at 0 looks like this:

$$
y_t^* = X_t \beta +\varepsilon_t,
$$
$$
y_t = 0 \hspace{0.5cm} \text{if} \hspace{0.2cm} y_t^* \leq 0,
$$

$$
y_t = y_t^* \hspace{0.5cm} \text{if} \hspace{0.2cm} y_t^* > 0.
$$

Here, $y_t^*$ is called the *latent dependent variable*, $X_t$ is a set of independent variables, $\beta$ is a set of coefficients, $\varepsilon_t$ are the residuals and $y_t$ is the actual dependent variable of the Tobit model.
...
```{r "5.1.6",fig.width=10.5,fig.height=7}
#< task
linesize = 1
...
```{r "5.1.10",fig.width=10.5,fig.height=7}
#< task
library(ggplot2)
library(patchwork)
...
#>
```
#< award "Living in a parallel universe"
Great, we have reached the end of the first exercise on counterfactual scenarios. Here is a comic about a contrafactual but funny universe.
...
```{r "5.2.5",fig.height=8, fig.width=9}
#< task
...
#>
```
#< award "Und täglich grüßt das Murmeltier"
Congratulations on your perseverance. The data set must also be loaded again for this last exercise. I promise you this will be the last time.
#>
...
```{r "5.3.7",fig.height=8, fig.width=9}
#< task
...
We arrived at our last major analysis result.
#>
...
```
#< award "Thank you!"
Thank you for your interest in this RTutor problem set.  I hope you were able to take away some interesting aspects from this energy-related topic and the *R* tasks.

I would like to thank everyone who supported me during the creation of this work. 
#>
...
Energy Information Administration (EIA) (2021): *California’s curtailments of solar electricity generation continue to increase*. <https://www.eia.gov/todayinenergy/detail.php?id=49276>. Accessed 29.05.2024.
...
Höhne, N. E., Braun, N. et al. (2012): *Greenhouse gas emission reduction proposals and national climate policies of major economies: Policy Brief*. Netherlands Environmental Assessment Agency, Bilthoven and ECOFYS, Utrecht.

International Renewable Energy Agency IREA (2023) – processed by Our World in Data: *“Global installed renewable energy capacity by technology”*. <https://ourworldindata.org/grapher/installed-global-renewable-energy-capacity-by-technology>. Accessed 29.05.2024.
...
NRG Editorial Voices (2023): *Electricity markets: what’s the difference between a wholesale energy market and a capacity market?* <https://www.nrg.com/insights/energy-education/electricity-markets-what-s-the-difference-between-a-wholesale-en.html>. Accessed 29.05.2024.

Ritchie, H. & Rosado, P. & Roser, M. (2020): *“Emissions by sector: where do greenhouse gases come from?”*. <https://ourworldindata.org/emissions-by-sector>. Accessed 29.05.2024.
...
`dplyr`: Wickham, H. & François, R. & Henry, L. & Müller, K. & Vaughan, D. (2023): *Package 'dplyr': A Grammar of Data Manipulation*. R package version 1.1.2. <https://github.com/tidyverse/dplyr> , <https://dplyr.tidyverse.org>.
